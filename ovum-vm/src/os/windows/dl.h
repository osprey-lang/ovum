...
namespace ovum
{
	...
	namespace os
	{
		...
		inline LibraryStatus OpenLibrary(const pathchar_t *path, LibraryHandle *output)
		{
			HMODULE library = ::LoadLibraryExW(
				path,
				nullptr,
#ifdef LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR
				// Let the native module come with its own libraries if it needs to
		...
		inline LibraryStatus CloseLibrary(LibraryHandle *library)
		{
		...
		inline bool LibraryHandleIsValid(LibraryHandle *library)
		{
		...
		inline void *FindLibraryFunction(LibraryHandle *library, const char *name)
		{

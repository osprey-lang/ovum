...
TYPED_ENUM(StringFlags, uint8_t)
{
	STR_NONE = 0,
...
typedef struct String_S
{
	...
	// The string's hash code. 0 is taken to mean "not calculated".
	// NOTE: if the hash function produces the hash code 0, the hash code
	// will be recalculated every time it's needed.
	int32_t hashCode;
	// If the flag contains STR_STATIC, the STRING is never garbage collected,
...
// And then you can safely cast LiteralString* to STRING*!
template<int Len>
class LitString
{
public:
	...
	{
		LitString<Len> output = { Len, 0, STR_STATIC };

		// Note: we can't use memcpy or CopyMemory because sizeof(char) != sizeof(uchar).
		for (int i = 0; i < Len; i++)

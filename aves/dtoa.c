 ...
 * Chirp.
 *
 * File downloaded on November 1, 2013.
 * http://www.netlib.org/fp/dtoa.c
 *
 * Concise list of modifications done to this file in order to fit
 * better with Ovum and the aves project:
 *  -  dtoa renamed to _aves_dtoa
 *  -  strtod renamed to _aves_strtod
 *  -  freedtoa renamed to _aves_freedtoa
 *  -  KR_headers option removed; we're in the future now!
 *  -  USE_LOCALE option removed; not needed for aves (at least not right now)
 *  -  macro CONST removed; all references replaced by 'const' keyword
...
#pragma warning(disable : 4703) // Potentially uninitialized variable used
                                // (spans entire file, see bottom for pop)
...
struct BCinfo { int dp0, dp1, dplen, dsign, e0, inexact, nd, nd0, rounding, scale, uflchk; };
...
#ifdef __cplusplus
extern "C" double _aves_strtod(const char *s00, char **se);
extern "C" char *_aves_dtoa(double d, int mode, int ndigits, int *decpt, int *sign, char **rve);
#endif
...
static void hexdig_init(void)	/* Use of hexdig_init omitted 20121220 to avoid a */
...
char *_aves_dtoa(double dd, int mode, int ndigits, int *decpt, int *sign, char **rve)
{
...
#pragma warning(pop) // Potentially uninitialized variable used
